"""
Fire Detection Algorithm Simulator
Simulates the ESP32-CAM fire detection algorithm on dataset
and analyzes false alarm sources and suppression mechanisms
"""
...
class FireDetector:
    ...
    @staticmethod
    def is_fire_colour(r, g, b):
        """Check if pixel color matches fire"""
        ...
        if delta == 0:
            return False
        if r < b + FireDetector.MIN_RED_DOMINANCE:
            return False
        if g > r + 20:
            return False
        
        sat = int((delta / max_c) * 255) if max_c > 0 else 0
        if sat < FireDetector.MIN_SATURATION:
            return False
        
        hue = FireDetector.calc_hue(r, g, b, max_c, delta)
        return FireDetector.hue_is_flame(hue)
    ...
    def analyse_frame(self, frame_rgb):
        ...
        for y in range(0, h, step):
            for x in range(0, w, step):
                r, g, b = frame_rgb[y, x]
                r, g, b = int(r), int(g), int(b)  # Ensure integer types
                total += 1
    ...
    try:
        with open(label_path, 'r') as f:
            ...
            if lines:
                ...
                return {
                    'class': int(parts[0]),  # 0 = fire, 1 = no fire (typically)
...
def run_simulation():
    """Run fire detection simulation on dataset"""
    ...
    for img_path in tqdm(image_files):
        ...
        ground_truth = label is not None and label['class'] == 0 if label else False
...
def create_table_1(df, metrics):
    ...
    table_data.append({
        ...
        'Effectiveness': 'Dark pixels rejected before color analysis'
    })
    ...
    table_data.append({
        ...
        'Threshold Mechanism': 'δ = max - min > 0 (color delta)',
        'Pixels Filtered': f'{fp_df["rejection_grey"].sum()}',
        'Effectiveness': 'Grayscale pixels rejected (no color info)'
    })
    ...
    table_data.append({
        ...
        'Optical Artifact': 'Hues outside flame color range (0-50°, 162-180°)',
    ...
    table_data.append({
        ...
        'Threshold Mechanism': f'FLICKER_THRESH = {FireDetector.FLICKER_THRESH}, Var window = {FireDetector.HIST_LEN}',
        'Pixels Filtered': 'Temporal analysis',
        'Effectiveness': 'Variance flicker detection eliminates transients'
    })
    
    # 6. Confirmation counter
    table_data.append({
        'False Alarm Source': 'Frame-level false positives',
        'Optical Artifact': 'Single frame meeting fire criteria',
        'Threshold Mechanism': f'CONFIRM_FRAMES = {FireDetector.CONFIRM_FRAMES}',
        'Pixels Filtered': f'Confirmation mechanism',
        'Effectiveness': f'Requires {FireDetector.CONFIRM_FRAMES} consecutive frames'
    })
...
def main():
    ...
    print(f"Flicker Threshold: {FireDetector.FLICKER_THRESH}")
